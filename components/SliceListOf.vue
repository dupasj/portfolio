 <template>
  <Container :screen="props.slice.primary.screen" :container="props.slice.primary.container">
    ...
    <div class="mt-[30px] xl:mt-[90px] grid grid-cols-2 xl:grid-cols-3 gap-x-[15px] md:gap-x-[30px] gap-y-[44px]">
      <NuxtLink  :to="{ name: `${type}-uid___${locale}`, params: { uid: item.uid } }" class="max-w-[370px]" v-for="item in items" :key="item.id">
        ...
        <button class="inline-block scroll-animation mt-[5px] md:mt-[7px] w-auto h-auto text-white text-[14px] md:text-[16px] font-medium m-0 p-0 font-sans subpixel-antialiased">
          {{$t('more')}}
        </button>
      </NuxtLink>
    </div>
