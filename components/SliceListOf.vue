 <template>
  <Container :screen="props.slice.primary.screen" :container="props.slice.primary.container">
    ...
    <div class="mt-[30px] xl:mt-[90px] grid grid-cols-2 xl:grid-cols-3 gap-x-[15px] md:gap-x-[30px] gap-y-[44px]">
      <NuxtLink  :to="{ name: `${type}-uid___${locale}`, params: { uid: item.uid } }" class="no-underline max-w-[370px]" v-for="item in items" :key="item.id">
        ...
        <h4 class="no-underline scroll-animation mb-[10px] mt-[10px] block w-full h-auto text-white text-[20px] md:text-[30px] font-medium m-0 p-0 font-sans subpixel-antialiased">{{ $prismic.asText(item.data.title) }}</h4>
        <p class="no-underline scroll-animation mt-[6px] block w-full h-auto max-h-[60px] text-clip overflow-hidden text-white text-[13px] md:text-[16px] font-medium m-0 p-0 font-sans subpixel-antialiased leading-[20px]">{{ $prismic.asText(item.data.ellipsis) }}</p>
        <button class="no-underline inline-block scroll-animation mt-[5px] md:mt-[7px] w-auto h-auto text-white text-[14px] md:text-[16px] font-medium m-0 p-0 font-sans subpixel-antialiased">
          {{$t('more')}}
        </button>
      </NuxtLink>
    </div>
