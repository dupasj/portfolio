<template>
  <Container :padding="false">
    <div class="blur-inner-container">
      <div class="left space-y-[30px]">
        <a :href="item.target_link.url ?? item.file.url ?? null" v-for="item of slice.items" class="w-full relative xl:pl-[76px] pl-[60px] no-underline">
          <div v-appear class="absolute left-0 top-[50%] -translate-y-1/2 xl:w-[56px] xl:h-[56px] w-[45px] h-[45px] rounded-full border border-[#52486C] flex justify-center items-center content-center justify-items-center">
          ...
          <p v-appear class="no-underline w-full h-auto xl:text-[20px] text-[16px] text-white xl:leading-[27px] leading-[20px] m-0 p-0">{{ transform(item.title)  }}</p>
          <span v-appear class="no-underline w-full h-auto xl:text-[30px] text-[20px] text-white xl:leading-[40px] leading-[30px] m-0 p-0">{{ transform(item.action_text)  }}</span>
        </a>
      </div>
        <div class="right">
          <form class="form" @submit="submit">
            <h5 v-appear>{{ $t('contact') }}</h5>
            <div v-appear class="input-field">
              <input ref="name" @blur="updateForm" @input="updateForm" type="text" autocomplete="off" name="name" :placeholder="$t('content')"/>
            </div>
            <div v-appear class="input-field">
              <input ref="email" @blur="updateForm" @input="updateForm" type="email" autocomplete="off" name="email" :placeholder="$t('email')"/>
            </div>
            <div v-appear class="input-field">
              <textarea ref="content" @blur="updateForm" @input="updateForm" type="text" name="content" :placeholder="$t('content')"/>
            </div>

            <div class="mt-[35px] w-full flex items-center content-center justify-end justify-items-end">
              <button v-appear :disabled="!active || loading" class="text-[#fff] bg-[#FF0058] disabled:bg-[#6B527C] rounded-[3px] pt-[7px] pb-[7px] pr-[12px] pl-[12px] text-[18px]" type="submit">
                {{  $t('send') }}</button>
            </div>
          </form>
        </div>
...
export default {
  ...
  methods: {
    transform(input){
      const raw = this.$prismic.asText(input);
      if (raw.toLowerCase().trim().endsWith("mon cv") && this.$i18n.locale === "en"){
        return "Download my CV";
      }
      ...
      this.$nextTick(async () => {
        ...
        try {
          ...
          await this.$axios.$post('/mail/send', {
            ...
            subject: 'Portefolio - ' + payload.name,
    ...
    updateForm(){
      this.active = (this.$refs.name.value.trim().length > 0 && this.$refs.email.value.trim().length > 0 && this.$refs.content.value.trim().length > 0);
    }
  },
  directive: {
...
<style scoped>
.blur-inner-container{
  position: relative;
  @apply w-full block xl:grid xl:grid-cols-2;
