    ...
    <div class="blur-inner-container">
      <div class="left space-y-[30px]">
        <a :href="item.target_link.url ?? item.file.url ?? null" v-for="item of slice.items" class="w-full relative xl:pl-[76px] pl-[60px] no-underline">
          ...
          <p v-appear class="no-underline w-full h-auto xl:text-[20px] text-[16px] text-white xl:leading-[27px] leading-[20px] m-0 p-0">{{ $prismic.asText(item.title)  }}</p>
          <span v-appear class="no-underline w-full h-auto xl:text-[30px] text-[20px] text-white xl:leading-[40px] leading-[30px] m-0 p-0">{{ $prismic.asText(item.action_text)  }}</span>
        </a>
      </div>
        <div class="right">
