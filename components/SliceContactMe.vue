    ...
    <div class="blur-inner-container relative w-full block xl:grid xl:grid-cols-2 rounded-[0px] xl:rounded-[30px]">
      <div class="left space-y-[30px]">
        <a :href="item.target_link.url ?? item.file.url ?? null" v-for="item of props.slice.items" class="w-full relative xl:pl-[76px] pl-[60px] no-underline">
          ...
          <p class="scroll-animation no-underline w-full h-auto xl:text-[20px] text-[16px] text-white xl:leading-[27px] leading-[20px] m-0 p-0">{{ transform(item.title)  }}</p>
          <span class="scroll-animation no-underline w-full h-auto xl:text-[30px] text-[20px] text-white xl:leading-[40px] leading-[30px] m-0 p-0">{{ transform(item.action_text)  }}</span>
        </a>
      </div>
        <div class="right">
...
const transform = (input) => {
  const raw = app.$prismic.asText(input);
  if (raw.toLowerCase().trim().endsWith("mon cv") && app.$i18n.locale === "en"){
